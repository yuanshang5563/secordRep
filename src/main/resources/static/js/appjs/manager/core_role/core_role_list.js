...
function load() {
	$('#coreRoleTable').bootstrapTable({
		method : 'get', // 服务器数据的请求方式 get or post
		url : prefix + "/coreRoleListJsonData", // 服务器数据的加载地址
...
function reLoad() {
	$('#coreRoleTable').bootstrapTable('refresh');
}
function add() {
	// iframe层
	layer.open({
		...
		area : [ '800px', '520px' ],
		content : prefix + '/coreRoleForm?actionType=add' // iframe的url
	});
}

function edit(coreRoleId) {
	layer.open({
		type : 2,
		title : '角色修改',
		maxmin : true,
...
function remove(coreRoleId) {
	layer.confirm('确定要删除选中的记录？', {
		btn : [ '确定', '取消' ]
	}, function() {
		$.ajax({
			url : prefix + "/deleteCoreRole",
			type : "post",
			data : {
